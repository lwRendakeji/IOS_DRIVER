...
#define SYSTEM_VERSION_LESS_THAN(v) ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumbericSearch] == NSOrderedAscending)
...
@synthesize txtLat;
@synthesize txtLng;
@synthesize txtLng_old;
@synthesize txtLat_old;
@synthesize txtAlt;
//@synthesize href;
...
- (void)viewDidLoad
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        //获取服务器version
        /*self.callMethod = @"chkversion";
        NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@:%@/%@/version.xml",self.webservice_id,self.webservice_port,self.webservice_name]];
        NSURLRequest *req = [[NSURLRequest alloc] initWithURL:url];
        NSURLConnection *conn = [[NSURLConnection alloc] initWithRequest:req delegate:self];
        NSMutableData *responseData = [[NSMutableData alloc]init];
        if(conn){
            responseData = [NSMutableData data];
            NSLog(@"service_version:%@",responseData);
        }*/
    });
...
-(void)locationManager:(CLLocationManager *)manager didFailWithError:(NSError *)error{
    ...
    if(lastLocateTime == nil || [@"" isEqualToString:lastLocateTime] || labs([[[lastLocateTime componentsSeparatedByString:@":"] objectAtIndex:0] intValue] - [d hour]) >= 2){
        ...
        //[self soap:@"LogService" methodName:@"baiduMapLog" fields:[Util transDictionaryToSoapMessage:dictionary]];//APP_ID  is ''
    }
}
...
-(void) connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    /*if([theXML rangeOfString:@"</version>"].location != NSNotFound){
         ...
         NSRange range2 = [theXML rangeOfString:@"</version>"];
         //app版本
         NSString *appVersion = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
         [theXML substringToIndex:range2.location];
         NSString *newVersion = [[theXML substringToIndex:range2.location] substringFromIndex:range1.location+range1.length];
        User *user = [[User alloc] init];
         if([newVersion floatValue] > [appVersion floatValue]){
         //有新版本升级
             [user setUserInfo:@"newVersion" value:@"true"];
             //[[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@:%@/%@/IOS_DRIVER.ipa",self.webservice_id,self.webservice_port,self.webservice_name]]];
         }else{
             [user setUserInfo:@"newVersion" value:@"false"];
         }
    }else */
...
-(BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    NSString *js_fit_code = [NSString stringWithFormat:@"$('body').css('padding-bottom','%@');document.activeElement.blur();document.documentElement.style.webkitTouchCallout='none';document.documentElement.style.webkitUserSelect='none';",self.scaleSize];
    ...
    if([[requestString uppercaseString] rangeOfString:@"PHOTO://"].location != NSNotFound){
        ...
        NSString *urlStr = [[NSString alloc] initWithFormat:@"%@?cityid=%@&key=%@",httpUrl,httpArg,APIKEY];
        ...
        return NO;
    }else if([[requestString uppercaseString] rangeOfString:@"UPDATEVERSION://"].location != NSNotFound){
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@:%@/%@/IOS_DRIVER.ipa",self.webservice_id,self.webservice_port,self.webservice_name]]];
        return NO;
        ...
        if(hasBaiduMap){
            ...
            [alert show];
            self.latlngArr = [[NSArray alloc] init];
            self.latlngArr = arr;
        }
        return NO;
    }
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    switch (buttonIndex) {
        case 0:
            break;
        case 1:
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://itunes.apple.com/cn/app/bai-du-tu-zhuan-ye-shou-ji/id452186370?mt=8"]];
            break;
        case 2:{
            CLLocationCoordinate2D coords2 = CLLocationCoordinate2DMake([[self.latlngArr objectAtIndex:3] floatValue],[[self.latlngArr objectAtIndex:2] floatValue]);
            MKMapItem *currentLocation = [MKMapItem mapItemForCurrentLocation];
            MKMapItem *toLocation = [[MKMapItem alloc] initWithPlacemark:[[MKPlacemark alloc] initWithCoordinate:coords2 addressDictionary:nil]];
            toLocation.name=@"Destination";
            [MKMapItem openMapsWithItems:[NSArray arrayWithObjects:currentLocation,toLocation,nil] launchOptions:[NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:MKLaunchOptionsDirectionsModeDriving,[NSNumber numberWithBool:YES],nil] forKeys:[NSArray arrayWithObjects:MKLaunchOptionsDirectionsModeKey,MKLaunchOptionsShowsTrafficKey,nil]]];}
