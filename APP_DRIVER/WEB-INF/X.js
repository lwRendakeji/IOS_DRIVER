  ...
  window.setInterval(function(){
                     ...
                     data.ADDRESS = X.cookie.get('address');
                     data.CITY = X.cookie.get('cityname');
                     data.LAT = X.cookie.get('lat');
                     data.LNG = X.cookie.get('lng');
                   
                     X.soap(data,'LogService','baiduMapLog');
                     },1000*60*30);
...
baidunav:function(v1,v2,v3,v4){
    if(X.isEmpty(v1) || X.isEmpty(v2) || X.isEmpty(v3) || X.isEmpty(v4)){
        X.dialog('定为信息不足,无法导航');
        return;
    }
    window.location = 'BAIDUNAV://'+v1+'//'+v2+'//'+v3+'//'+v4;
},
/*
 * 获取天气,单词有误,暂不修改
 */
...
dialog:function(resultMsg){
    if(typeof resultMsg == 'undefined' || resultMsg == 'undefined') return;
		...
		table.find('thead').find('tr').find('th').each(function(){
			if(!X.isEmpty(this.id)){
				if(this.id == 'TRS_ID' || this.id == 'SO_ID'){
					...
					tr.find('td').eq($(this).index()).text(result[i][this.id].replace('null',''));
				}
